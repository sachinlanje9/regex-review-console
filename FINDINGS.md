...
columns per row. Scope of this pass: **DEBIT** — 4,762,821 transactions across
829 regexes.

23 distinct defects across 4 themes. Each has the observed behaviour, why it
...
**Legend** — impact on downstream use:
`CLUSTER` inflates or fragments regex clusters ·
...
### A1. Missing start/end anchors `CLUSTER`
...
### A2. Duplicate regexes for the same format `CLUSTER`
...
### A3. Delimiters splitting one transaction across clusters `CLUSTER`
...
### A4. A field split by its own delimiter spawns extra regexes `CLUSTER`
...
### A5. `[ ]` used as the delimiter class `CLUSTER` `EXTRACT`
...
### A6. Clusters generated with no valid features `CLUSTER`
...
### B1. The same feature appears multiple times in one map `EXTRACT`
...
### B2. Group names are not in order of occurrence `EXTRACT`
...
### B3. Not all feature columns are present in the map `EXTRACT`
...
### C1. Incomplete transaction captured before field extraction `EXTRACT`
...
### C2. Counterparty truncated by spaces and special characters `EXTRACT`
...
### C3. Counterparty split across groups by delimiters `EXTRACT`
...
### C6. Ambiguous values forced into a typed field `EXTRACT`
...
### C7. Multiple reference numbers from one transaction `EXTRACT`
**Fix:** one `ref_num` per transaction unless the format genuinely carries more.

### C8. Reference number matched as a single digit `EXTRACT`
...
### C9. Multiple modes from one transaction `EXTRACT`
**Fix:** exactly one `mode` per transaction.

### C10. Invalid datetime patterns `EXTRACT`
...
### D2. `CW` mapped to the wrong field `RULES`
`CW` denotes **flow**, but is extracted as counterparty or another feature.

**Fix:** map `CW` → `payment_flow`.

### D3. `CR` / `DR` not treated as flow in AEPS transactions `RULES`
**Fix:** map CR/DR to flow for AEPS.

### D4. `DR` extracted as a standalone feature `RULES`
**Fix:** `DR` always belongs in `remark`.

### D5. Business keywords hardcoded out of counterparty and remark `EXTRACT` `RULES`
...
### D6. `CASH` hardcoded out of a counterparty name `EXTRACT` `RULES`
...
### D7. `SALARY` in keyword position `RULES`
Should not automatically become a keyword feature.

**Fix:** handle explicitly in L2 with a business rule.

### D8. Reversal keyword at the start of a note not captured `EXTRACT` `RULES`
...
### D9. Fixed deposits not tagged `RULES`
```
FDR 03/9604 CLOSURE
...
Cross-cutting root causes, in the order I'd fix them:
